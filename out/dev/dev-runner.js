...
async function emptyMainOutput() {
  ...
  const files = await (0, _util().orNullIfFileNotExist)((0, _fsExtraP().readdir)(outDir));

  if (files == null) {
    return;
  }

  await _bluebirdLst().default.map(files.filter(it => !it.startsWith(".") && it !== "main.js"), it => (0, _fsExtraP().remove)(outDir + path.sep + it));
}
